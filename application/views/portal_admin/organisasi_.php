...
<article class="module width_full">
	...
	$(document).ready(function() {
		$(".flexme3").flexigrid({
			id : 'admin_grid',
			url : 'index.php/portal_admin/organisasi/get_organisasi',
			dataType : 'json',
			sortname : "kategori_id",
			sortorder : "asc",
			usepager : true,
			title : 'Content List',
			useRp : true,
		...
		function ajaxDelete (id) {
			...
			$.ajax({
				type : 'POST'
				,url : 'index.php/portal_admin/organisasi/delete_organisasi'
				,data : 'id='+id
		...
		function procEdit(celDiv,id)
		{
			...
			(
				function ()
				{
					window.location = "index.php/portal_admin/organisasi/update_organisasi/"+id;
				}
			);
		} 
		...
		function procDelete(celDiv,id)
		{
			...
			(
				function ()
				{
					...
					var answer = confirm('Anda yakin akan menghapus konten '+kategori+' ?');
					if (answer) {
						ajaxDelete(id);
					} else {
						return false;
		...
		function test(com, grid) {
			if (com == 'Delete') {
				...
				window.location = "index.php/portal_admin/organisasi/add_organisasi";
			}
		}
